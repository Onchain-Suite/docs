...
<ParamField path="recipientMode" type="string">
  `triggering_profile` (default) sends to the contact who entered the automation.
  `wallet_list` sends to a fixed list of wallets.
</ParamField>
...
In-app pushes reach wallets that have authenticated through the SDK. Offline users receive the push on their next connect, within the 72-hour retention window. See [In-App Notifications](/integrations/in-app-notifications) for the client setup, and note that in-app has its **own monthly allowance**, separate from email.
