...
```html CDN loader
<!-- inapp.js bundles everything it needs and auto-starts from data-* attributes -->
<script
...
```html CDN (ESM)
<!-- ESM import: load the real-time client first so the SDK can find it on window.io -->
...
export function OnchainInApp({ wallet }: { wallet: string | null }) {
  ...
  useEffect(() => {
    ...
    const os = new OnchainSuite(process.env.NEXT_PUBLIC_ONCHAIN_PK!, {
      apiBaseUrl: "https://api.onchainsuite.com",
      // Bridge signing to your wallet library so WalletConnect and mobile
      // wallets work, not just injected ones.
      signMessage: (message) => signMessageAsync({ message }),
    });

    os.on("connected", () => console.log("onchain: connected"));
    os.on("notification", (n) => {/* render your toast or inbox */});
    os.start(wallet);

    return () => os.stop(); // clean up on unmount or wallet change
  }, [wallet]);
...
return <>{address && <OnchainInApp wallet={address} />}</>;
```
...
const os = new OnchainSuite(process.env.NEXT_PUBLIC_ONCHAIN_PK!, {
  apiBaseUrl: "https://api.onchainsuite.com",
  display: false, // your UI, not the built-in toast
  onNotification: (n, actions) => {
    actions.report("viewed"); // it's about to be shown
    showToast({
      title: n.title,
      body: n.body,
      cta: n.cta, // { label, url } | undefined
      onClickCta: () => actions.click(), // reports "clicked" and opens the url
      onClose: () => actions.dismiss(), // reports "dismissed"
    });
    return false; // suppress the built-in toast
  },
});

await os.start(wallet);
```
...
interface Notification {
  deliveryId: string; // stable id, use it to dedupe
  title: string;
  body: string;
...
interface NotificationActions {
  report: (type: "delivered" | "viewed" | "dismissed" | "clicked") => void;
  click: () => void;   // reports "clicked" and opens cta.url
  dismiss: () => void; // reports "dismissed"
}
```

Reporting maps to analytics and [webhooks](/api/webhooks): arrival is `message.delivered`, `report("viewed")` is `message.viewed`, and `click()` is `message.clicked`.
...
Mobile push reaches a native iOS or Android app even when it's closed. It needs a one-time setup, because each notification travels through Apple's and Google's own push services, and they have to trust your app first.

**What you need from your side:**

<Steps>
  <Step title="Set up push with Apple and Google">
    - **iOS:** an Apple Developer account and an **APNs auth key** (a `.p8` file).
    - **Android:** a Firebase project with **Firebase Cloud Messaging (FCM)** enabled, and its service-account key.
  </Step>
  <Step title="Upload those credentials once, in the dashboard">
    An Owner or Admin adds them under your organization's push credentials. Until they're in place, device tokens register but nothing is delivered.
  </Step>
  <Step title="Get the device token in your app, and hand it to the SDK">
    The SDK doesn't fetch the token for you. Get it with your own push library (for example `expo-notifications` or `@react-native-firebase/messaging`) and pass it to `registerDevice`.
  </Step>
</Steps>

**What this does:** ties one device to a wallet so pushes can reach it. Call it after `start()`.
...
await client.registerDevice(token, "android"); // use "ios" for Apple devices

// Tokens rotate, so keep the registration fresh.
messaging().onTokenRefresh((t) => client.registerDevice(t, "android"));
```

Use `"ios"` for Apple devices (APNs) and `"android"` for Android (FCM). Stop delivery to one device with `unregisterDevice(token)`.
...
- **Your keys.** See your publishable keys (safe to ship in the browser) and the details of your secret keys, the name, when each was last used, and whether it's been revoked. More in [API keys](/integrations/api-keys).
