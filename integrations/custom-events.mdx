...
<Note>
**In plain terms.** If you've used a product-analytics or email tool, this is the familiar "track an event" idea: you tell Onchain Suite "this person just did this thing," and then you can message or segment on it. Because it uses your secret key, a developer wires this into your backend once, and marketing builds the automations on top.
</Note>
...
Sending an event is one request that says *who* did *what*. It's authenticated with a **secret key**, so it runs from your server (never the browser), and the workspace is worked out from the key.
...
Idempotency is a long word for a simple safety net: it stops the same event being counted (and acted on) twice when a request gets retried.
