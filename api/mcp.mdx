...
Because the assistant reads the actual reference tables, it won't invent column names or trigger types, which is the failure mode these docs exist to prevent.
...
- **The Onchain Suite API** is a REST API. It does not expose an MCP endpoint. See [API Overview](/api/overview) and [API Keys](/integrations/api-keys).
