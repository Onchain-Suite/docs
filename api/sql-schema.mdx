...
<Note>
**In plain terms.** This is the map of your data: the tables you can query and what each column holds. If you're writing SQL in the Intelligence editor, or asking the AI to generate it for you, this is the reference it draws on. You only ever read data here, never change it.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Unsupported table reference(s)" icon="table">
    Every name after `FROM` or `JOIN` is checked against the eleven tables above, including names you define yourself. Alias your subqueries rather than referencing them as tables.
  </Accordion>
