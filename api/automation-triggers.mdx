...
<Note>
The queued and batch endpoints apply a short **finality delay** before matching — a per-chain wait (seconds on an L2, up to a couple of minutes on Ethereum mainnet) so a re-orged transaction never fires a trigger. The two synchronous endpoints (`/triggers/onchain-event`, `/triggers/goldrush-event`) match immediately with no delay, so send those only for events you already consider final.
</Note>
