...
Optional fields let you match more precisely: `topic0` (the event signature hash), `programId` and `instructionName` (Solana), and `sourceId` to tag the origin. The platform resolves the canonical event from whatever you supply, preferring `topic0`, then Solana program plus instruction, then the event name.
