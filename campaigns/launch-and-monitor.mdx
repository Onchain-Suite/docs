...
You can also [schedule](/campaigns/overview#scheduling) a launch for later, the scheduler runs it for you at the set time, through the same checks.

Sending is asynchronous: the campaign moves to **Sending**, is delivered in batches, then moves to **Sent**. Large sends may be paced across the day rather than delivered in one burst.

## Pausing a send

You can pause a campaign while it's sending. Pausing **stops its queued messages**, not just the status, so anything not yet dispatched is held. Resume to continue from where it stopped. This is the kill switch for a send that's going wrong, for example a bad link you only noticed after launch.
...
Sent and delivered counts, unique opens, total and unique clicks, the negative outcomes (bounces, complaints, unsubscribes, suppressed, and failed), and the rates that matter: delivery, bounce, open, click, unsubscribe, and click-to-open.

Delivery and bounce rate are two shares of the same **delivery attempts**, so they add up to 100%, which matches how your email provider reports them.
...
The campaigns list shows each campaign's opens, clicks, and open and click rates inline, computed over its full audience, so you can scan performance without opening each one.
